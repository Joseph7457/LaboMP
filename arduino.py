...
test = sortie_memorisee(valeur_memorisee)
...
def dessiner_afficheur(sortie_CD4511):
    ...
    for barre in positions_barres:
        if sortie_CD4511[i] == 0:
            ...
            continue
        x_b = pos_afficheur[0] + \
            int(round(barre[0]*(image_afficheur.get_width()/133)))
        y_b = pos_afficheur[1] + \
            int(round(barre[1]*(image_afficheur.get_height()/192)))
...
image_afficheur_s = pygame.image.load(
    'images/7_seg_s.png').convert_alpha(fenetre)
barre_verticale_s = pygame.image.load(
    'images/vertical_s.png').convert_alpha(fenetre)
barre_horizontale_s = pygame.image.load(
    'images/horizontal_s.png').convert_alpha(fenetre)
image_afficheur = pygame.image.load('images/7_seg.png').convert_alpha(fenetre)
barre_verticale = pygame.image.load(
    'images/vertical.png').convert_alpha(fenetre)
barre_horizontale = pygame.image.load(
    'images/horizontal.png').convert_alpha(fenetre)
...
while True:
    ...
    sortie_CD4511 = composant_CD4511(sortie_memorisee(valeur_memorisee))
    dessiner_afficheur(sortie_CD4511)
    dessiner_arduino(sortie_memorisee(valeur_memorisee),
                     sortie_CD4511, sortie_bouton)
